...
function SearchContent() {
    ...
    return (
        <div className="p-8 pt-4 min-h-full font-geist-sans selection:bg-ekko-500/30">
            {/* Floating Header */}
            <div className="mb-10 sticky top-3 z-40 bg-black/60 backdrop-blur-xl py-4 md:py-5 rounded-none border border-white/5 shadow-2xl">
                ...
                {!query ? (
                    <div className="flex flex-col items-center justify-center mt-10 animate-fade-in-up">
                        <div className="text-center mb-16 space-y-4">
                            <div className="inline-flex items-center gap-2 border border-ekko-500/30 bg-ekko-500/10 px-3 py-1.5 text-ekko-300 text-[10px] uppercase tracking-widest rounded-full mb-4">
                                <span className="relative flex h-2 w-2">
                                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-ekko-400 opacity-75"></span>
                                    <span className="relative inline-flex rounded-full h-2 w-2 bg-ekko-500"></span>
                                </span>
                                System Index: Live
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-4xl">
                            {[
                                { name: "Tagalog Hits", img: "/playlist-liked.png" },
                                { name: "Pinoy Soul", img: "/playlist-discover.png" },
                                { name: "Opm Vibe", img: "/playlist-daily-mix.png" },
                                { name: "Island Lo-Fi", img: "/community-connect.png" },
                                { name: "Future Funk", img: "/hero-main.png" },
                                { name: "Ambient", img: "/agos-ghibli.png" },
                                { name: "City Pop", img: "/bayanihan-ghibli.png" },
                                { name: "Electronic", img: "/digital-village.png" }
                            ].map((vibe) => (
                                <MagicCard
                                    key={vibe.name}
                                    gradientColor="#6366F1"
                                    className="h-24 rounded-none overflow-hidden"
                                >
                                    <button
                                        onClick={() => router.push(`/search?q=${encodeURIComponent(vibe.name)}`)}
                                        className="group relative w-full h-full bg-black/50 overflow-hidden hover:border-ekko-500/50 transition-all duration-300 active:scale-95"
                                    >
                                        <Image
                                            src={vibe.img}
                                            alt={vibe.name}
                                        ...
                                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                            <ArrowUpRight className="w-4 h-4 text-ekko-400" />
                                        </div>
                                ...
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between text-sm group">
                                        <div className="flex items-center gap-2 text-neutral-400">
                                            <span className="font-mono text-ekko-400">{activity.user}</span>
                                            <span>{activity.action}</span>
                                            <span className="text-white font-medium group-hover:text-ekko-400 transition-colors cursor-pointer">{activity.target}</span>
                                        </div>
                    ...
                    <div className="flex flex-col animate-fade-in-up">
                        ...
                        {results && results.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {results.map((song: any, i: number) => (
                                    <div
                                        ...
                                        role="button"
                                        tabIndex={0}
                                        className="group relative bg-neutral-900/40 hover:bg-neutral-900/80 border border-white/5 hover:border-ekko-500/30 rounded-none p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer"
                                        onClick={() => setQueue(results, i)}
                                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setQueue(results, i) } }}
                                    >
                                        <div className="aspect-square rounded-none bg-neutral-950 mb-4 relative overflow-hidden shadow-lg border border-white/5">
                                            {song.coverUrl ? (
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 backdrop-blur-[2px]">
                                                <div className="w-12 h-12 rounded-full bg-ekko-500 text-white flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.4)] transform scale-90 group-hover:scale-100 transition-transform">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <h3 className="font-bold text-white leading-tight truncate px-1 group-hover:text-ekko-300 transition-colors">{song.title}</h3>
                            ...
                            <div className="flex flex-col items-center justify-center py-20 text-neutral-600 border border-dashed border-neutral-800 rounded-none">

...
export function TrackInfo() {
    ...
    return (
        <div className="flex items-center gap-3 md:gap-4 w-full overflow-hidden">
            ...
            {/* Cover Art */}
            <div
                className={cn("relative h-14 w-14 rounded-none overflow-hidden bg-neutral-800 shrink-0 group hover:scale-105 transition-transform cursor-pointer", isRadio && "shadow-[0_0_15px_rgba(34,197,94,0.4)]")}
            ...
            <div className="flex flex-col overflow-hidden min-w-0">
                <div className="flex items-center gap-2">
                    <span
                        className={cn("text-sm font-medium truncate cursor-pointer hover:underline decoration-neutral-400", isRadio && "text-green-400")}
                        onClick={handleExpand}
                    >
                        {display.title}
                    </span>
                ...
                <span className="text-xs text-muted-foreground truncate flex items-center gap-1">
                    {isRadio && <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />}
                    {display.artist}
                </span>
            </div>
        </div>

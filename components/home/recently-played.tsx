        ...
        <section className="mb-12 relative z-10">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-black tracking-tight text-white/90 flex items-center gap-3">
                    <span className="bg-cyan-500 w-1.5 h-6 rounded-full" />
                    Recently Played
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {recentSongs.map((song) => {
                    ...
                    return (
                        <div
                            key={song.id}
                            className={`group flex items-center gap-4 glass-card rounded-xl p-3 cursor-pointer transition-all duration-300 hover:bg-white/8 hover:scale-[1.01] border border-white/5 hover:border-white/10 ${isCurrentlyPlaying ? 'bg-blue-600/10 border-blue-500/30' : ''
                                }`}
                            onClick={() => setQueue(songs, songs.findIndex(s => s.id === song.id))}
                        >
                            {/* Album Art */}
                            <div className="relative w-14 h-14 rounded-lg overflow-hidden bg-neutral-900 flex-shrink-0 shadow-lg">
                                <Image
                                    src={song.coverUrl}
                                    alt={song.title}
                                    fill
